...
export async function putSalary(userId: string, entry: { entryId?: string; year: number; amount: number; note?: string }) {
  const client = getDocClient();
  if (!client) throw new Error('DynamoDB table not configured');
  const now = new Date().toISOString();
  const id = entry.entryId || (typeof crypto !== 'undefined' && (crypto as any).randomUUID ? (crypto as any).randomUUID() : Date.now().toString());
  const item = {
    pk: `user#${userId}`,
...
export async function deleteSalary(userId: string, entryId: string, year: number) {
  const client = getDocClient();
  if (!client) throw new Error('DynamoDB table not configured');
